...
public class I2COpSet {
    public enum OUTPUT_TYPE{DEC,HEX,BIN,CHAR, BITS, VALIDCHAR, NONE}
    private String bitsRegex;
    ...
    public void setOutputType( String radix ){
        ...
        outType = switch( radix.toLowerCase()){
            ...
            case "bin","binary" -> OUTPUT_TYPE.BIN;
            case "char" -> OUTPUT_TYPE.CHAR;
            case "okchar","validchar" -> OUTPUT_TYPE.VALIDCHAR;
            case "bits" -> OUTPUT_TYPE.BITS;
    ...
    public boolean isAscii(){
        return outType==OUTPUT_TYPE.CHAR||outType==OUTPUT_TYPE.VALIDCHAR;
    }
    /**
    ...
    private String parseDoubleResult(ArrayList<Double> altRes){
        ...
        switch (outType) {
            ...
            case BITS ->
                altRes.forEach(x -> {
                            var binary = Integer.toBinaryString(x.intValue());
                            var padded = String.format(bitsRegex, binary).replace(' ', '0');
                            output.add( String.join(";", padded.split("")) );
                        });
            case CHAR -> {
                var line = new StringJoiner("");
                altRes.forEach(x -> line.add( String.valueOf((char) x.intValue())));
                Logger.info("Data read: "+line);
                output.add(line.toString());
            }
            case VALIDCHAR ->{
                    return altRes.stream().mapToInt(Double::intValue)
                                            .filter( x->x<128)
