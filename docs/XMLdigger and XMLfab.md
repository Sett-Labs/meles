...
A lot of meles relies on XML for setting up etc, so a lot of code interacting with those files is used.
...
class XMLTest {
    public XMLTest {
        var fab = XMLfab.withRoot(Path.of("settings.xml"), "meles");
        if( fab.isInvalid() ) {
...
class Example {
    public boolean addAnother() {
        var dig = XMLdigger.goIn(Path.of("settings.xml"), "meles");
        dig.digDown("streams");
