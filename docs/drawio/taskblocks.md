...
- **Block:** A shape in the diagram representing a unit of functionality, defined by its properties (`melestype`,
  `melesid`, etc.).
- **Block Type:** The content of the `melestype` property.
...
- `Command:` Often abbreviated to `cmd`, text that is part of the Command Pipeline to control meles but can also be use
...
- **Shape Properties and Arrow Label Are Key:** All logic depends on:
    - The properties (mainly `melestype`) of the shapes.
...
- **Required properties:**
    - `melestype`: Must be `basicvalblock` for it to be processed as a Basicval Block.
...
- **Required properties:**
    - `melestype`: Must be `clockblock` for it to be processed as a Clock Block.
...
- **Required properties:**
    - `melestype`: Must be `conditionblock` for it to be processed as a Condition Block.
...
- **Required property:**
    - `melestype`: Must be `controlblock` for it to be processed as a Control Block.
...
- **Required properties:**
    - `melestype`: Must be `counterblock` for it to be processed as a Counter Block.
...
- **Incoming Arrow**: From another task block, triggers the start of the delay countdown. Successive triggers restart
  the countdown.
- **Required properties:**
    - `melestype`: Must be `delayblock` for it to be processed as a Delay Block.
...
- **Required properties:**
    - `melestype`: Must be `flagblock` for it to be processed as a Flag Block.
...
- **Required properties:**
    - `melestype`: Must be `intervalblock` for it to be processed as an Interval Block.
...
- **Required properties:**
    - `melestype`: Must be `logblock` for it to be processed as a Log Block.
...
- **Required properties:**
    - `melestype`: Must be `mathblock` for it to be processed as a Math Block.
...
- **Required properties:**
    - `melestype`: Must be `originblock` for it to be processed as an Origin Block.
    - `melesid`: An unique ID used to reference the task/sequence. It needs to be unique within the file.
- **Optional Properties:**
  ...
  - `shutdownhook`: Defaults to `no`/`false`. Determines if the task is run during the shutdown process of meles.
...
- **Outgoing Arrows**:
    ...
    - Labeled `error` (or `failure`), diverting to an alternative route if the source is invalid.
- **Incoming Arrows**:
...
- **Required properties:**
    - `melestype`: Must be `readerblock` for it to be processed as a Reader Block.
    - `wants`: Defines the data to wait for. Default check is equals and case-insensitive.
  ...
  - `source`: Defines the source of the data fe. raw:sensor, email:read.
- **Optional Properties:**
    ...
    - `check`: If the default of equals and ignore case as a check isn't correct, this can be changed.
      - `equals` The received data must exactly match `wants`.
      - `equalsignorecase` The received data must match `wants` ignoring case differences.
      - `startswith`: The received data must start with `wants`.
      - `contains` The received data must contain `wants`.
      - `regex` The received data must match the `wants` regex pattern.
...
- **Required properties:**
    - `melestype`: Must be `triggergateblock` for it to be processed as such.
...
- **Incoming Arrow**:
    - From another task block, triggers to resolve the target and write the data. Successive triggers repeat the
      writing.
- **Required properties:**
    - `melestype`: Must be `writerblock` for it to be processed as a Writer Block.
