...
One of the main components of meles is the taskmanager functionality.

Reset the settings.xml back to this and restart meles:
...
<meles>
    <settings>
        ...
        <telnet port="23" title="meles"/> <!-- The telnet server is available on port 23 and the title presented is DAS-->
    </settings>
    <streams>
...
<task output="email:admin" trigger="delay:5s">meles booted;Nothing else to say...</task>
        <!-- This will send an email to admin 5s after startup, subject of the email is meles booted and the content 'Nothing...' -->
