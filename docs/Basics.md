...
> **Note: Document up to date for 1.0.x**

The purpose of this (probably in the end very long) page is to slowly introduce the different components in meles and
how to use them.
The basis will be interacting with a dummy sensor that simulates rolling a d20 (a 20 sided die). This wil be simulated
by another instance of meles running on the same system. Do note that practicality isn't the mean concern, showing what
is (or isn't) possible is.

The dummy sensor is in fact just meles running a purpose made settings.xml and script.  
...
> meles doesn't have a GUI and uses a telnet server for interaction instead.
...
* To shut down the instance of meles, `sd:reason` fe. `sd:updating to new version`

### Setup & startup

1. Download the latest release version of meles from
   the [meles releases page](https://github.com/michieltjampens/meles/releases). Pick the zip file that contains all
   files.
2. Extract it to a working folder, make a second copy of it and rename that one to dummy. Keep the original.
3. Download
   the [Diceroller]([examples/Diceroller.zip](https://github.com/michieltjampens/meles/raw/main/examples/simulations/diceroller/diceroller.zip))
   package
4. Extract the content into the dummy folder (fe. settings.xml should be on the same level as the .jar)
5. From now on, the meles version that generates the dummy (diceroller) data will be called "dummy", the other one we'll
   call "regular"
6. Start both the regular and dummy meles (double-click on their respective .jar files). If your firewall (Windows
   Defender, ...) ask permissions you'll have to grant them.
...
> Note: Dummy can be accessed via telnet if needed, it is listening on port 2324 instead of the standard 2323. meles
> refuses to start if there is a telnet active on port 23. This prevents duplicate instances running.

### Workspace layout

The following is the recommended workspace layout for setting up meles in general:

* Have two meles telnet instances open
  * Open PuTTY, select telnet as the protocol, use `localhost` as the ip, change the port to 2323.
    * Localhost means that meles is running on your PC/laptop 'locally'.
    * Type 'meles' in the 'saved session' box and click 'Save'.
...
* Keep the settings.xml open and visible, [notepad++](https://notepad-plus-plus.org/downloads/v8.5/) is a suitable lightweight editor
  * It's possible to have it auto-update on changes done by meles
...
## Interacting with meles

The earlier workspace layout connected to meles.
...
If this doesn't help, create an [issue](https://github.com/michieltjampens/meles/issues) about it (and maybe attach
those logs)
...
Use `st` to see the current state of meles. This will give you a lot of info including a section about streams which
...
What we did so far (starting up meles and connecting to a stream) generated a `settings.xml` file that looks like this (
...
<meles>
  <settings>
    ...
    <telnet port="2323"
            title="meles"> <!-- The telnet server is available on port 2323 and the title presented is meles -->
...
* Anything starting with \<word> and closing with \</word>, is called a node (the text in it a 'tag').
  * The first such node is called the rootnode, so <meles> ... </meles> (tag is meles).
  * A node inside another node is called a childnode, so settings is a childnode of meles and so on.
...
* Start with `store:dice,!!` because all commands that we'll use start with that.
  * This tells meles that all the following things we'll send need to have everything in front of the '!!' appended.
...
* Next up is adding an integer `store:streamid,addint,id<,index>`
  ...
  * There's no command to create an empty store, meles just makes one on the first 'add' cmd
...
Everytime a store command is issued, meles applies those changes.
...
```xml
    <databases>
      <sqlite id="rolls" path="db\rolls.sqlite"> <!-- This will be an absolute path instead -->       
        ...
        <idleclose>-1</idleclose> <!-- Do note that this means the file remains locked till meles closes -->
...
rtval in meles. The default rtval used has id `tablename_columnname`.
...
id of the stream is used. Below is how the store is actually read by meles:
```xml
<stream id="dice" type="tcp">
  <eol>crlf</eol>
  <address>localhost:4000</address>
...
> Note: this just serves to show how to add a server to meles, this won't install said database server
...
Then use the `sd` command to shut down meles and start a new instance (launch the `.jar`).

### 1. Creating a path

As the name implies, 'a' path defines 'the' path the data travels in meles.
...
<path delimiter=":" id="dice" src="raw:dice">
  ...
  <!-- because the previous step was a 'store', meles assumes you're done with the filtered data -->
...
A major aspect of meles is the concept of source and writable. Source means 'it can provide data' while writable means
...
* To shut down the instance of meles, `sd:reason` fe. `sd:updating to new version` or just `sd`
...
So a lot can be omitted and then meles will just assume the default values (which are hard coded).  
...
What was shown so far was when meles needs to figure out the relationship between a table and a store.  
...
So first close meles and dummy, delete the rolls.sqlite file and overwrite the xml to match the one below.  
...
Start both dummy and then meles again.
...
This tells meles to look for d20s_rolld20 instead of rolls_rolld20 etc.
